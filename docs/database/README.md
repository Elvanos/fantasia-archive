...
| File | Scope |
|------|--------|
| [projectDB.md](projectDB.md) | Active **`.faproject`** SQLite: `PRAGMA user_version` (max **11**), `project_data` KV, content tables (incl. **`tags`** / **`document_tags`**, **`document_last_opened`**, **`media`** type/link/embed + v10 **`internal_type`** + v11 **`external_embed`**), migration entry, main-process modules, **`FA_PROJECT_CONTENT_IPC`**, **Project Settings** worlds snapshot save (per-world template layout) |
