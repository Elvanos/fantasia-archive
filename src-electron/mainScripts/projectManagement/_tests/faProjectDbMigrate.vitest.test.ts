...
function applyMockUserVersionPragma (
  ...
  if (name === 'user_version = 10') {
    pragmas.user_version = 10
  }
  if (name === 'user_version = 11') {
    pragmas.user_version = 11
  }
}
...
test('applyFaProjectMigrations bootstraps schema when user_version is 0', () => {
  ...
  expect(pragmas.user_version).toBe(11)
...
test('applyFaProjectMigrations migrates user_version 10 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
  ...
  const pragmas: Record<string, unknown> = { user_version: 10 }
  const db = {
    exec: (sql: string) => {
      execCalls.push(sql)
    },
  ...
  applyFaProjectMigrations(db as never, 'Realm')
  expect(pragmas.user_version).toBe(11)
  expect(execCalls.some((sql) => sql.includes('external_embed'))).toBe(true)
  expect(execCalls.some((sql) => sql.includes("'embed'"))).toBe(true)
})

test('applyFaProjectMigrations migrates user_version 4 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
...
test('applyFaProjectMigrations migrates user_version 5 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
...
test('applyFaProjectMigrations migrates user_version 6 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
...
test('applyFaProjectMigrations migrates user_version 8 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
  ...
  expect(execCalls.some((sql) => sql.includes('linked_outside'))).toBe(true)
  expect(execCalls.some((sql) => sql.includes('external_embed'))).toBe(true)
...
test('applyFaProjectMigrations migrates user_version 7 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
...
test('applyFaProjectMigrations migrates user_version 1 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
...
test('applyFaProjectMigrations migrates user_version 2 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
...
test('applyFaProjectMigrations migrates user_version 3 to 11', () => {
  const run = vi.fn((fn: () => void) => {
    return () => {
      fn()
    }
