...
const KEEP_EDIT_E2E_FAPROJECT = 'e2e-save-keep-edit-persist.faproject'
const KEEP_EDIT_E2E_PROJECT_NAME = 'E2E save keep edit persist project'
const KEEP_EDIT_E2E_ORIGINAL_LABEL = 'E2E Keep Edit Original'
const KEEP_EDIT_E2E_SAVED_LABEL = 'E2E Keep Edit Saved'
const OPENED_DOCUMENTS_PERSIST_SETTLE_MS = 750

let e2eKeepEditDocumentId = ''
...
async function createE2eProjectOnWorkspaceRoute (
  ...
  await e2eSetNextProjectCreatePath(electronApplication, KEEP_EDIT_E2E_FAPROJECT)
  await page.locator(`[data-test-locator="${selectorList.splashNew}"]`).click()
  await expect(page.locator(`[data-test-locator="${selectorList.nameInput}"]`)).toBeVisible()
  await page.locator(`[data-test-locator="${selectorList.nameInput}"]`).fill(KEEP_EDIT_E2E_PROJECT_NAME)
  await page.locator(`[data-test-locator="${selectorList.createBtn}"]`).click()
  await e2eExpectFaActiveProjectStoreName(page, KEEP_EDIT_E2E_PROJECT_NAME)
...
test.describe.serial('Opened documents E2E — keep-edit save before cold restart', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    const launched = await launchFaPlaywrightE2eAppWindow({
      afterIsolationResetBeforeLaunch (): void {
        tryUnlinkE2eFaprojectFixture(KEEP_EDIT_E2E_FAPROJECT)
      },
  ...
  test('Keep-edit save keybind saves name and stays in edit mode', async () => {
    await createE2eProjectOnWorkspaceRoute(appWindow, electronApp)
    e2eKeepEditDocumentId = await seedPreviewPersistedOpenedDocumentTab(appWindow)
    expect(e2eKeepEditDocumentId.length).toBeGreaterThan(0)

    await hydrateOpenedDocumentsAndRoute(appWindow, e2eKeepEditDocumentId)
    await expect(
    ...
    await nameInput.fill(KEEP_EDIT_E2E_SAVED_LABEL)
    ...
    ).toContainText(KEEP_EDIT_E2E_SAVED_LABEL)

    await appWindow.waitForTimeout(OPENED_DOCUMENTS_PERSIST_SETTLE_MS)
  })
})

test.describe.serial('Opened documents E2E — cold restart keeps keep-edit session', () => {
  let electronApp: ElectronApplication
  let appWindow: Page
  let suiteTestInfo: TestInfo
  ...
  test('Cold restart restores edit mode with keep-edit saved name', async () => {
    expect(e2eKeepEditDocumentId.length).toBeGreaterThan(0)
