...
const EDIT_SAVE_E2E_FAPROJECT = 'e2e-edit-save-keybinds.faproject'
const EDIT_SAVE_E2E_PROJECT_NAME = 'E2E edit save keybinds project'
const EDIT_SAVE_E2E_ORIGINAL_LABEL = 'E2E Edit Save Original'
const EDIT_SAVE_E2E_KEEP_EDIT_LABEL = 'E2E Edit Save Keep'
const EDIT_SAVE_E2E_FINAL_LABEL = 'E2E Edit Save Final'
const OPENED_DOCUMENTS_PERSIST_SETTLE_MS = 750

let e2eEditSaveDocumentId = ''
...
async function createE2eProjectOnWorkspaceRoute (
  ...
  await e2eSetNextProjectCreatePath(electronApplication, EDIT_SAVE_E2E_FAPROJECT)
  await page.locator(`[data-test-locator="${selectorList.splashNew}"]`).click()
  await expect(page.locator(`[data-test-locator="${selectorList.nameInput}"]`)).toBeVisible()
  await page.locator(`[data-test-locator="${selectorList.nameInput}"]`).fill(EDIT_SAVE_E2E_PROJECT_NAME)
  await page.locator(`[data-test-locator="${selectorList.createBtn}"]`).click()
  await e2eExpectFaActiveProjectStoreName(page, EDIT_SAVE_E2E_PROJECT_NAME)
...
test.describe.serial('Opened documents E2E — edit and save keybinds before cold restart', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    const launched = await launchFaPlaywrightE2eAppWindow({
      afterIsolationResetBeforeLaunch (): void {
        tryUnlinkE2eFaprojectFixture(EDIT_SAVE_E2E_FAPROJECT)
      },
  ...
  test('Edit and save keybinds rename document and exit edit mode', async () => {
    await createE2eProjectOnWorkspaceRoute(appWindow, electronApp)
    e2eEditSaveDocumentId = await seedPreviewPersistedOpenedDocumentTab(appWindow)
    expect(e2eEditSaveDocumentId.length).toBeGreaterThan(0)

    await hydrateOpenedDocumentsAndRoute(appWindow, e2eEditSaveDocumentId)
    await expect(
    ...
    const nameInput = appWindow.locator(
      `[data-test-locator="${selectorList.documentNameInput}"]`
    )
    await expect(nameInput).toBeVisible({ timeout: 15_000 })

    await nameInput.fill(EDIT_SAVE_E2E_KEEP_EDIT_LABEL)
    await appWindow.keyboard.press(getFaPlaywrightDefaultSaveDocumentKeepEditModePressString())
    await expect(nameInput).toBeVisible()
    await expect(nameInput).toHaveValue(EDIT_SAVE_E2E_KEEP_EDIT_LABEL)

    await nameInput.fill(EDIT_SAVE_E2E_FINAL_LABEL)
    ...
    await expect(
      appWindow.locator(`[data-test-locator="${selectorList.documentPreviewTitle}"]`)
    ).toHaveText(EDIT_SAVE_E2E_FINAL_LABEL)
    await expect(
      appWindow.locator(
        `[data-test-locator="projectAppControlBar-tab-${e2eEditSaveDocumentId}"]`
      )
    ).toContainText(EDIT_SAVE_E2E_FINAL_LABEL)
    await appWindow.waitForTimeout(OPENED_DOCUMENTS_PERSIST_SETTLE_MS)
  })
})

test.describe.serial('Opened documents E2E — cold restart keeps renamed preview title', () => {
  let electronApp: ElectronApplication
  let appWindow: Page
  let suiteTestInfo: TestInfo
  ...
  test('Cold restart restores renamed document in preview mode', async () => {
    expect(e2eEditSaveDocumentId.length).toBeGreaterThan(0)

    await navigateFaPlaywrightE2eToSplashRoute(appWindow)
    await clickFaPlaywrightE2eSplashResumePrimarySegment(appWindow)
    await e2eExpectFaActiveProjectStoreName(appWindow, EDIT_SAVE_E2E_PROJECT_NAME)
    ...
    await expect(
      appWindow.locator(`[data-test-locator="${selectorList.documentPreviewTitle}"]`)
    ).toHaveText(EDIT_SAVE_E2E_FINAL_LABEL, { timeout: 15_000 })
    await expect(
      appWindow.locator(
        `[data-test-locator="projectAppControlBar-tab-${e2eEditSaveDocumentId}"]`
      )
