...
| Project Media | **`DialogProjectMedia`** — control bar (after tree, inside Hide function buttons) / Content menu after Quick-Search / keybind **`openProjectMedia`** / action **`openProjectMediaDialog`** (`{ initialPanel? }` known id wins; missing/invalid → **`mediaAdd`** if project has no media, else **`mediaList`**); sticky **`q-dialog`** (`persistent`; Close only; same-keybind switches panel, no toggle-close); four **`q-tab-panels`** (**`mediaList`** / **`mediaAdd`** / **`mediaSingleEdit`** / **`mediaMassEdit`**); **`mediaAdd`** dashed drop zone + **Add offline media** / **Add online media** (Import/Export-style; no picker/CRUD yet); **Search media...** on **`mediaList`** only; no list/CRUD |
