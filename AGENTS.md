...
Not required to build/ship app. **caveman** → [`.cursor/rules/caveman-default.mdc`](.cursor/rules/caveman-default.mdc). **YAGNI** minimal code inside repo guardrails → [`.cursor/rules/yagni.mdc`](.cursor/rules/yagni.mdc), [fantasia-yagni](.cursor/skills/fantasia-yagni/SKILL.md). **cavemem** → global CLI + [`.cursor/hooks.json`](.cursor/hooks.json). Hooks **fail-open**. **Arbor** code graph → local gitignored **`.cursor/mcp.json`**, [fantasia-arbor.mdc](.cursor/rules/fantasia-arbor.mdc), [fantasia-arbor](.cursor/skills/fantasia-arbor/SKILL.md).
...
| [fantasia-arbor.mdc](.cursor/rules/fantasia-arbor.mdc) | Always — project Arbor MCP callers/impact; Grep fallback |
...
| Arbor code graph | [fantasia-arbor.mdc](.cursor/rules/fantasia-arbor.mdc), [fantasia-arbor](.cursor/skills/fantasia-arbor/SKILL.md) — project MCP; Grep if down |
...
| Domain script barrels | **`faDragDrop_manager`**, **`dom_manager`**, **`faColorContrast_manager`**, **`documentAppearance_manager`**, **`faColorPicker_manager`**, **`faSelectInput_manager`**, **`faProjectMedia_manager`**, **`openedDocuments_manager`** — [typescript-scripts.mdc](.cursor/rules/typescript-scripts.mdc), [fa-two-level-architecture.mdc](.cursor/rules/fa-two-level-architecture.mdc) |
...
| Project Media | **`DialogProjectMedia`** — control bar (after tree, inside Hide function buttons) / Content menu after Quick-Search / keybind **`openProjectMedia`** / action **`openProjectMediaDialog`** (`{ initialPanel? }` known id wins; missing/invalid → **`mediaAdd`** if no media, else **`mediaList`**); per-tab **`q-dialog` `persistent`** (active slide only; same-keybind switches panel, no toggle-close); five **`q-tab-panels`** (**`mediaList`** / **`mediaAdd`** / **`mediaAddOnlineUrls`** / **`mediaSingleEdit`** / **`mediaMassEdit`**); per-slide **h5** titles (**Project Media - Media list** on **`mediaList`**; **Project Media - Single medium edit** on **`mediaSingleEdit`**; **Project Media - Add New Media**; **Project Media - Add Online Media**; **Project Media - Mass Edit / Add Media**); **`mediaList`** sticky; **`mediaSingleEdit`** sticky iff loaded row dirty; slide-out ESC closes iff clean and no edit field focused (first ESC blurs; capture-stop so sticky dialog does not shake); **`mediaAdd`** ESC-close; **`mediaAddOnlineUrls`** sticky iff URL textarea nonempty (trim), else ESC-close; **`mediaMassEdit`** always sticky; **`mediaAdd`** dashed drop zone + **Add offline media** / **Add online media** (Import/Export-style; no picker/CRUD yet); **Add online media** slides to **`mediaAddOnlineUrls`** (h6 **Add any numbers of your media URLs, each on a new line.** + textarea; footer **Close** + **Submit media list** right-aligned; reset draft + focus textarea on enter; **Submit media list** disabled while draft blank/whitespace; no back); Submit skips blank lines, appends session rows, switches to mass-edit; mass-edit **rows** (themed horiz divider between; 136px square thumb with #62615c well and 6px radius from visible URL; image / unknown / text path extensions → bitmap fill square (cover); video/audio → **`video_file`** / **`audio_file`** (cultured **`accent`**) after file exists; External Type **Embed** → **`fa-solid fa-file-code`** (cultured **`accent`**) when **External embed body** nonempty; blank/missing file or empty embed body → warning-colored warning icon (fantasy gradient) + warning frame + tooltip **Medium not found / Invalid URL**, hide until success, URL probe delayed, leftover URL ignored for Embed, save still enabled; no card frame; Title; Type + Internal Type or External Type; Internal link or External link or External embed body; unused pair hidden; URL disablement unchanged; Embed textarea 4-line min, full width, grow taller only; one list scrollbar at dialog right edge; mass-edit footer **Close without saving** + **Save media and back to list** + **Save media and close** like Project Settings; **Save media and back to list** / **Save media and close** upsert media by id via action **`saveProjectMedia`**, success/error toast, then **`mediaList`** or close; **Close without saving** clears rows; slide switch does not; list slide loads **`listMedia`** into 150px thumb grid newest **created_at_ms** first (same preview/warning as mass-edit thumbs; WP-style title overlay, URL fallback if name blank; overlay updates after slide **Save**; same #62615c well); left-click thumb opens **700px** right slide-out (max-width 100%; slide-out q-input / FaSelectInput / textarea no Quasar dense (mass-edit rows + single-edit tab still dense); full-width 300px preview (contain image; playable video; audio control bar in 300px well with preload auto, 20px side inset, relayout after slide transform; raw embed HTML height initial, overflow hidden, well corner radius); no heading; form centered in slide; left **Previous** / **Next** arrows (40px outline; aria-label only, no hover tooltip; dirty disables both; first disables **Previous**, last disables **Next**; **ArrowLeft** / **ArrowRight** match those buttons unless an edit field is focused; same dirty / animating / end locks as clicks; nav duplicates body; **Previous** incoming from the left, **Next** from the right); **Close** / **Save without close** / **Save and close**; panel + backdrop animate in/out; backdrop blocks clicks, no dismiss; dirty **Close** discards; **Save without close** upserts via **`saveProjectMedia`**, fail stays, success reloads list and keeps slide; **Save and close** upserts via **`saveProjectMedia`**, fail stays, success closes slide + reload list); hide dialog **Close** while slide or **`mediaSingleEdit`** tab; tab **Close** discards + close dialog, **Save and close** save then close, Save disabled if no row; **Search media...** on **`mediaList`** only; search filter not wired |
...
| App theme | **`appTheme`** enum (**`types/faUserSettingsAppThemeRegistry.ts`**) — flat/fantasy × light/dark; **`applyFaAppThemeToDocument`** → **`body.fa-appTheme--flat|fantasy`**, **`body--dark|light`**, **`data-fa-app-theme`**, Quasar **`Dark`**; store **`S_FaUserSettings`**; palette **`$fantasy-dark`** / **`$fantasy-medium`**; ripples on globally, fantasy hides via CSS; textarea ink **`--fa-content-fields-text-color`** (`fa-theme.scss`; not Custom App CSS **`--fa-color-*`** list) |
...
| `fantasia-arbor` | Project Arbor MCP — callers, callees, impact, map |
