...
| DB | **`better-sqlite3`**; **`.faproject`** SQLite **`user_version` max 10** — see [projectDB.md](docs/database/projectDB.md) |
...
| Project Media | **`DialogProjectMedia`** — control bar (after tree, inside Hide function buttons) / Content menu after Quick-Search / keybind **`openProjectMedia`** / action **`openProjectMediaDialog`** (`{ initialPanel? }` known id wins; missing/invalid → TEMP **`mediaMassEdit`** (revert later: **`mediaAdd`** if no media, else **`mediaList`**)); sticky **`q-dialog`** (`persistent`; Close only; same-keybind switches panel, no toggle-close); four **`q-tab-panels`** (**`mediaList`** / **`mediaAdd`** / **`mediaSingleEdit`** / **`mediaMassEdit`**); **`mediaAdd`** dashed drop zone + **Add offline media** / **Add online media** (Import/Export-style; no picker/CRUD yet); **Add online media** swaps drop zone for URL textarea + **Submit media list** (no back; Close or other slide to leave; Submit skips blank lines, appends session rows, switches to mass-edit; URL form stays until Close); mass-edit **QTable** (Title / Type / Internal Type / External Type / Internal link / External link; **Save media changes** unwired; Close clears table; slide switch does not; no DB load); **Search media...** on **`mediaList`** only; no list/CRUD |
